...
export const Signature: FC<SignatureProps> = ({code}) => {
  ...
  } = useSWRImmutable((signMessageData && address) ? (`${dev_hostname}/nestfi/op/user/login?chainId=97&remember=true&walletAddress=${address}`) : undefined, (url: any) => fetch(url, {
    method: "POST",
    headers: {
  ...
  } = useSWRImmutable((jwt && code) ? `/api/dev/auth` : undefined, (url) => fetch(url, {
    method: "POST",
    headers: {
